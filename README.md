...
- 可选修复 H3.TextColor 长文本拆行后的跨行颜色继承，默认关闭
- 可选为右键弹出的通用滚动文本对话框（如魔法详情）提供鼠标滚轮滚动支持；默认关闭，左键弹窗保持原生滚动
...
- `[Features] EnableRightClickScroll` 控制右键魔法描述框滚轮翻页，默认关闭
- `[Features] EnableTextColorFix` 控制滚动文本跨行颜色补丁，默认关闭
...
如启用 `[Features] EnableTextColorFix=1`，MegaDesc 会补齐 `_DlgScrollableText_` 长文本拆行后的 `{~Color}` 颜色状态，避免魔法详细描述等滚动文本在换行后丢失自定义文字颜色。该选项默认关闭，未启用 `H3.TextColor.dll` 时也不作为硬依赖。
